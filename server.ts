...
async function startServer() {
  ...
  // Luxury AI Cake Recommendation Endpoint
  app.post("/api/ai-recommendation", async (req, res) => {
    const { category, guests, vibe, flavorPref, budget } = req.body;

    try {
      const prompt = `You are the lead Executive Pastry Chef at CakeUrban, designing a luxury custom cake for an elite client in Delhi NCR.
      Design a custom cake based on the following:
      - Occasion/Category: ${category || "General Celebration"}
      - Number of Guests: ${guests || "20"}
      - Vibe/Aesthetics: ${vibe || "Ultra-modern minimalism with gold accents"}
      - Flavor Preferences: ${flavorPref || "Belgian Dark Chocolate & salted caramel"}
      - Estimated Budget: ${budget || "₹5000"}

      Return a JSON response specifying a highly luxurious bespoke cake recommendation.
      Format the response exactly as a JSON object with these keys (output only clean JSON, no markdown formatting):
      {
        "name": "Name of this bespoke masterwork (Dior/Louis Vuitton level brand name)",
        "flavorCombo": "Detailed luxury flavor combination, description of layers and premium fillings",
        "tiersAndStructure": "Detailed architectural layout, tier counts, shape, and physical presence",
        "artisanDetails": "Artistic detailing, edible elements, gold leaves, drapery, sugar works, textures, or specific lighting/floating effects",
        "pairing": "Suggested high-end champagne, premium wine, or organic tea pairing",
        "price": "Suggested luxury price in INR (e.g., ₹4500)",
        "weight": "Suggested weight in kg (e.g., 2.5 kg)",
        "designInspiration": "A paragraph explaining the design inspiration matching Apple, Dior, or Nike design philosophies"
      }`;
      ...
      const fallback = {
        name: `${category || "Bespoke"} Royale`,
        flavorCombo: `${flavorPref || "Belgian Chocolate Truffle"} infused with Madagascan Vanilla Caviar and fleur de sel caramel.`,
        tiersAndStructure: "A majestic multi-tiered silhouette with hand-carved circular layers.",
        artisanDetails: "Adorned with hand-brushed 24-karat edible gold leaves, delicate sugar draping, and customized monogram embossing.",
        pairing: "Dom Pérignon Champagne or Grand Cru Premium Darjeeling Tea.",
        price: budget ? `₹${budget}` : "₹4,999",
        weight: "2.0 kg",
        designInspiration: "Inspired by Dior's architectural silhouette drape lines and Nike's focus on seamless performance and athletic precision."
      };
      res.json(fallback);
    }
  });
