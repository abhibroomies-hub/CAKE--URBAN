      ...
      if (u) {
        ...
        try {
          ...
          if (snap.exists()) {
            ...
            setProfile({
              uid: u.uid,
              displayName: u.displayName || 'Artisan User',
              email: u.email || '',
              role: 'customer'
            });
          }
        } catch (err) {
          console.warn("Failed to fetch user profile from Firestore, falling back to local metadata:", err);
          setProfile({
            uid: u.uid,
            displayName: u.displayName || 'Artisan User',
            email: u.email || '',
        ...
        try {
          ...
          console.warn("Failed to fetch local user profile from Firestore:", err);
          setProfile({
            uid: localUserObj.uid,
            displayName: localUserObj.displayName,
            email: localUserObj.email,
