...
import { NotFoundView } from '../components/FeedbackStates';
...
export default function LocationSEOPage() {
  ...
  // 404 Guard for completely nonsense slugs
  const validKeywords = [
    'delhi', 'noida', 'gurgaon', 'faridabad', 'ghaziabad', 'sector', 
    'bakery', 'cake', 'cupcake', 'dessert', 'biscuit', 'cookie', 
    'hamper', 'bento', 'pinata', 'pull-me-up', 'veg', 'eggless',
    'birthday', 'anniversary', 'wedding', 'kids', 'gifting', 'chocolate',
    'truffle', 'red-velvet', 'black-forest', 'butterscotch', 'pineapple',
    'mango', 'strawberry', 'hazelnut', 'biscoff', 'rasmalai'
  ];
  const isValid = LOCATION_DATA_MAP[routeKey] || validKeywords.some(kw => routeKey.toLowerCase().includes(kw));
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      ...
      (err) => {
        console.error("Failed to fetch products for LocationSEOPage:", err);
      }
    );
    return () => unsubscribe();
  }, []);
  ...
  const relevantProducts = React.useMemo(() => {
    ...
    if (slugLower.includes('hamper') || slugLower.includes('gift-box') || slugLower.includes('casket') || slugLower.includes('basket') || slugLower.includes('cookie-tray') || slugLower.includes('gifting')) {
      const filtered = dbProducts.filter(p => p.categories?.includes("Hampers") || p.categories?.includes("Curated Hampers & Gift Boxes"));
      if (filtered.length > 0) return filtered.slice(0, 4);
    }
    
    // 2. Bakery & Artisanal Breads
    if (slugLower.includes('sourdough') || slugLower.includes('bread') || slugLower.includes('focaccia') || slugLower.includes('baguette') || slugLower.includes('brioche') || slugLower.includes('muffin') || slugLower.includes('bakery') || slugLower.includes('pastry')) {
      const filtered = dbProducts.filter(p => p.categories?.includes("Pastries & Slices") || p.categories?.includes("Breads"));
      if (filtered.length > 0) return filtered.slice(0, 4);
    }
    ...
    if (slugLower.includes('chocolate') || slugLower.includes('truffle') || slugLower.includes('ferrero') || slugLower.includes('hazelnut')) {
      const chocolateProducts = dbProducts.filter(p => 
        p.flavors?.some((f: string) => f.toLowerCase().includes('chocolate')) ||
    ...
    if (slugLower.includes('velvet')) {
      const velvetProducts = dbProducts.filter(p => p.name.toLowerCase().includes('velvet'));
      if (velvetProducts.length > 0) return velvetProducts.slice(0, 4);
    }

    if (slugLower.includes('bento')) {
      const bentoProducts = dbProducts.filter(p => p.name.toLowerCase().includes('bento') || p.description?.toLowerCase().includes('bento'));
      if (bentoProducts.length > 0) return bentoProducts.slice(0, 4);
    }

    // Default Mix of Bestsellers
    return dbProducts.filter(p => p.isBestseller).slice(0, 4);
  }, [routeKey, dbProducts]);
