...
export default function AdminDashboard() {
  ...
  const [categoriesList, setCategoriesList] = useState<string[]>(() => {
    ...
    return [
      'Birthday Cakes',
      'Anniversary Cakes (Romantic & Milestone)',
      'Baby Shower & Welcome',
      'Engagement & Wedding',
      ...
      'Pinata Cakes',
      'Pull-Me-Up Cakes',
      'Designer Tiered Cakes',
      'Regular Cakes',
      'Pastries & Slices',
      'Cupcakes & Muffins',
      'Curated Hampers & Gift Boxes'
    ];
  });
  ...
  const handleClearAllProducts = () => {
    setConfirmAction({
      message: `Are you absolutely sure you want to remove ALL products from the database? This will clear the storefront completely. Auto-seeding will be disabled until you synchronize or manually add cakes again.`,
      onConfirm: async () => {
        const toastId = toast.loading('Emptying confections database & disabling auto-seeding...');
        try {
          const productsSnap = await getDocs(collection(db, 'products'));
          ...
          // Disable auto-seeding persistently in the cloud settings
          await setDoc(doc(db, 'settings', 'autoseed'), { disabled: true });
          // Disable locally in localStorage as a fallback
          localStorage.setItem('disable_autoseed', 'true');

          setProducts([]);
          setSelectedProductIds([]);
          
          toast.success(`Cleared all ${count} products. Storefront is now empty and auto-seeding is deactivated!`, { id: toastId });
        } catch (err) {
          console.error(err);
          toast.error('Failed to empty the database. Please check firestore write permissions.', { id: toastId });
        }
      }
    });
  };
